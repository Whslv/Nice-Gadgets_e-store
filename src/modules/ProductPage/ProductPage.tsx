...
export const ProductPage = () => {
  ...
  const category = () => {
    switch (typeOfProduct) {
      case 'phones':
        ...
        return phones;
      case 'tablets':
        const tablets = productData.filter(
      ...
      case 'accessories':
        const accessories = productData.filter(
          accessor => accessor.category === 'accessories',
        );
  ...
  const perPageNum =
    perPage === 'all' ? null : Math.max(1, parseInt(perPage, 10) || 1);

  const totalPages = perPageNum
    ? Math.max(1, Math.ceil(total / perPageNum))
    : 1;
  ...
  const productPerPage = perPageNum
    ? sortedProduct.slice(start, end)
    : sortedProduct;
